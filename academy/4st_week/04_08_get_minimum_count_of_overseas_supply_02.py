...
def get_minimum_count_of_overseas_supply(stock: int, dates: list, supplies: list, k) -> int:

    answer = 0
    lastAddedDate = 0
    maxHeap = []

    # 재고는 0에 도달할 수 없습니다. 가동이 중단될 수 없습니다.
    while stock <= k:
        # 모두 순화하기 전에 공급량이 떨어지기 전입니다.
        while lastAddedDate < len(dates) and dates[lastAddedDate] <= stock:
            # Max힙에 추가합니다.
            heapq.heappush(maxHeap, -supplies[lastAddedDate])
            lastAddedDate += 1
        answer += 1
        heapPop = heapq.heappop(maxHeap)
        stock += -heapPop
    return answer
