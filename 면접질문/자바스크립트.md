...
순수함수란주 언제든 몇 번이든 실행해도 결과가 같은 함수를 의미합니다. 순수함수는 함수 외부의 영향을 받지않고 또 영향을 주지도 않습니다. 순수함수는 주로 계산만 할 때 활용합니다. 인자는 자유롭게 바뀌어도 문제가 없습니다. 인자가 동일하면 반환도 동일해야 합니다. 즉 외부 영향을 받지도 주지도 않아야 합니다. 함수가 정의된 곳에서 렉시컬 환경에 있는 외부 변수에 의존하지 않고 매개변수만 의존하면 순수함수입니다. 반면 비순수함수는 동일한 인자를 대입해도 매번 다를 결과를 출력할지도 모르는 함수입니다. 클로져처럼 외부 변수에 의존, 제너리레이터, 서버와 통신에 매번 갱신된 데이터를 얻을지 모를 때, 등 순수한 계산과는 무관하고 어떤 기능을 활용해야 할 때 많이 활용합니다.

함수형 프로그래밍의 3가지 개념이 있습니다. 데이터, 액션, 계산인데 여기서 데이터는 최대한 변형을 가하지 않고 불변성을 지키기 위해 즉 원본에 변형을 가하는 것으로 사이드이펙트를 가하지 않기 위해 복제를 하고 액션을 처리합니다.
